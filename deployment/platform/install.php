...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$alreadyInstalled) {
    ...
    $adminName = clean_text($_POST['admin_name'] ?? '', 100);
    $adminEmail = filter_var(trim((string) ($_POST['admin_email'] ?? '')), FILTER_VALIDATE_EMAIL);
    $adminPassword = (string) ($_POST['admin_password'] ?? '');

    if (!$database || !$user || !$adminEmail || mb_strlen($adminName) < 2 || strlen($adminPassword) < 12) {
        $error = 'Remplissez tous les champs. Le mot de passe administrateur doit contenir au moins 12 caractères.';
    } else {
        try {
            $pdo = new PDO(
            ...
            $statement->execute([$adminName, strtolower((string) $adminEmail), password_hash($adminPassword, PASSWORD_DEFAULT)]);
...
</style></head><body><main class="card"><h1>Installation LINKSTECH</h1><p>Connectez la plateforme à la base MySQL créée dans cPanel, puis créez le premier compte administrateur.</p>
